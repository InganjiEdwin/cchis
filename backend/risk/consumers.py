...
from accounts.session_security import validate_access_token_session
...
NOTIFICATION_STREAM_SUBPROTOCOL = "cchis.notifications"


class DashboardNotificationConsumer(AsyncJsonWebsocketConsumer):
    async def connect(self):
        token, requested_subprotocol = self._extract_token()
        if not token:
        ...
        await self.accept(subprotocol=requested_subprotocol)
    ...
    def _extract_token(self) -> tuple[str | None, str | None]:
        subprotocols = [str(value).strip() for value in self.scope.get("subprotocols", []) if str(value).strip()]
        if NOTIFICATION_STREAM_SUBPROTOCOL in subprotocols:
            stream_token = next(
                (value for value in subprotocols if value != NOTIFICATION_STREAM_SUBPROTOCOL),
                "",
            )
            if stream_token:
