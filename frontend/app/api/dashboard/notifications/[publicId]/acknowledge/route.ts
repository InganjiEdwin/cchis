...
export async function POST(request: Request, context: { params: Promise<{ publicId: string }> }) {
  ...
  try {
    const payload = await fetchBackendJson(`/notifications/${publicId}/acknowledge/`, {
      method: "POST",
      cookieHeader,
    });
    ...
    return NextResponse.json({ detail: "Unable to acknowledge the notification." }, { status: 500 });
  }
}
