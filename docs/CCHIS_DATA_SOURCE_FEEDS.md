...
| Alerts and interfaces | Mobitech Bulk SMS is the active SMS provider path; the preserved Africa's Talking adapter is parked for later sandbox verification. | Dashboard, CHV, SMS/USSD, message governance, governed alert creation, Celery delivery, Mobitech acceptance persistence and idempotent delivery callbacks. | Stub mode is explicitly simulated; no provider acceptance or final delivery is claimed without a controlled test recipient and reachable callback/status reconciliation. |
