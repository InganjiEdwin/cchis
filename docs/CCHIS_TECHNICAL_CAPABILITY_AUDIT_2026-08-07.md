...
The audit used source inspection, backend API calls, a browser pass through the Next.js application, management commands, and the committed automated test suites. The 2026-08-08 registry correction was verified with focused registry and truth-policy tests only; the full-suite CI job remains the verifier for unrelated regressions.
...
The subsequent model-artifact-registry correction pass (2026-08-08) adds explicit candidate registration, SHA-256/size integrity evidence, approval requests/reviews, challenger designation, activation, retirement and explicit-target rollback transitions. Approval, activation, rollback-target validation and strict registry audit now invoke the persisted truth policy independently of `CCHIS_ENVIRONMENT`; governance actors resolve to active CCHIS users with role checks, self-approval is rejected, and event sequences are audited as append-only state transitions. Production scoring resolves the active `live_baseline` entry through a centralized gate, but the current training-on-demand path has no trusted registered-artifact loader and therefore remains blocked by `production_registered_inference_path_required`. It does not approve or activate a model. The local registry remains empty (`active_model_count=0`) and the read-only registry audit reports `NOT_APPROVED_FOR_OPERATIONAL_USE`; this document therefore remains a “not production ready” assessment.

Permitted claim: CCHIS implements a fail-closed model lifecycle registry with artifact integrity, source-backed dataset and feature-contract provenance, active-user governance identity, immutable event/state auditing, approval controls, challenger states, explicit rollback targets and a centralized production scoring gate. No current model is approved for operational use, and no registered-artifact inference path is claimed.

### Automated tests and build

| Area | Result |
|---|---|
| Backend Django suite | Baseline: **924 tests discovered; 924 passed**. Earlier correction verification: **937 tests discovered; 937 passed** in 610.734 seconds. The current registry correction was intentionally checked with focused suites only; the full suite is delegated to CI |
| Model artifact registry focused suite | `docker compose exec -T backend python manage.py test risk.test_model_artifact_registry --verbosity 1`: **22/22 passed** |
| Persisted truth-policy focused suite | `docker compose exec -T backend python manage.py test risk.test_truth_policy --verbosity 1`: **22/22 passed** |
| Registry correction integrity checks | `makemigrations risk --check --dry-run`, `migrate --check`, `check` and `docker compose config --quiet`: **passed** |
| Strict local registry audit | `audit_model_registry --strict`: **passed**; 0 registered entries, 0 governance/approval events, 0 active models, `operational_model_available=false`, readiness `NOT_APPROVED_FOR_OPERATIONAL_USE` |
...
| Database migrations | **Verified working** | 83 risk migration files are applied; migrate --check completed without pending work. |
...
| Model artifact registry | **Implemented, partially verified** | `ModelRegistryEntry` persists a stable registry version, model/run and source-backed dataset references, ordered feature contract, evaluation metrics, code commit, artifact location/format/size/SHA-256 and intended/prohibited use. The persisted truth evaluator is environment-independent; the strict audit validates actor identity, append-only event sequence and state transitions. Focused registry tests pass; the local registry has 0 entries and 0 active models, so no operational artifact is claimed. |
| Promotion governance | **Implemented, partially verified as a safety block** | Registration, approval request/review, challenger designation, activation, retirement and explicit-target rollback are separate active-user/reason transitions. Requester/reviewer separation, role checks, event immutability and database target constraints are enforced. The centralized production scoring gate validates a real active `live_baseline` entry but returns `production_registered_inference_path_required` until a trusted artifact loader exists. No current model is approved or active. |
...
| Model registry/promotion/rollback | **Implemented, partially verified** | Explicit artifact/lifecycle governance, source-backed strict truth checks, active-user authorization, immutable/sequenced events, integrity checks, read-only admin surface and focused tests exist; the local registry intentionally has no approved model, active target or rollback event. The trusted registered-artifact inference path remains an explicit operational gap. |
...
1. **Correction regressions covered.** The focused production truth-policy suite passes **22/22**, and the focused model-artifact-registry suite passes **22/22**, including source-backed acceptance, fake-live lineage rejection, artifact/scheme/root checks, strict local activation, self-approval/role rejection, concurrent activation and event sequence/mutation checks. The earlier correction full suite passed **937/937**; the current full suite is delegated to CI. Supersession/correction data quality still needs real-source validation.
