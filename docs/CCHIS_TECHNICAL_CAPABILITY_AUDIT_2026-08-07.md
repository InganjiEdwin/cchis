...
The audit used source inspection, backend API calls, a browser pass through the Next.js application, management commands, and the committed automated test suites.
...
| Production-like Django deployment check | Baseline check passed only with command-line environment overrides; the correction check passed with the resolved production Compose env file, provider wiring and no extra overrides |
...
| Area | Result |
|---|---|
| Backend Django suite | Baseline: **924 tests discovered; 924 passed**. Correction verification: **933 tests discovered; 933 passed** in 621.879 seconds |
| Surveillance truth/regression tests | Passed; explicit `as_of` timestamps now make historical fixtures deterministic while preserving rolling-window cutoffs and truth gates |
...
- Added a central production truth policy that blocks seeded/demo labels, static climate fallback, synthetic feature rows, invalid/unmapped wards, superseded canonical references and unsafe production model metadata before scores or alerts can be produced. Proxy evidence remains allowed as caveat context and cannot satisfy an explicit confirmed-truth claim.
...
| Model feature generation | **Implemented, partially verified** | Features include rainfall, flood indicator, historical cases, seasonality, population proxies, settlement concentration, floodplain exposure, water-body proximity and WASH vulnerability. Feature provenance and truth gates are persisted; production blocks seeded labels, static/synthetic fallback rows, invalid geography and stale canonical references. Proxy evidence remains available as caveat context but cannot be asserted as confirmed truth. |
...
| Production model truth gate | **Verified working as a safety block** | Seeded training/inference, seeded simulation lineage, static/synthetic rainfall or population fallback, invalid row geography, superseded references and demo model metadata are blocked in production. Blocked runs do not create scores or alerts and record stable reason codes for review. |
| Model artifact registry | **Planned/documented only** | describe_model_ops_state reported 0 registry entries, 0 promotion events and 0 rollback events. Training is performed in-process; no active registry target was available for rollback. |
| Promotion governance | **Verified working as a safety block** | Registry admission and alert generation reject production runs with truth-policy blockers; seeded/demo runs remain blocked, benchmark-only or preview-only. This is a safety control, not evidence of a promoted model. |
...
| Model registry/promotion/rollback | **Planned/documented only** | Governance metadata and blocking rules exist; no active registry, promotion event or rollback target. |
...
The correction pass makes the production profile self-consistent: `DEBUG`, HSTS include-subdomains/preload, Mailgun and Africa's Talking variables are resolved through Compose, and live providers fail startup when required credentials are absent. The CI acceptance command is `docker compose --env-file deploy/ci-production.env run --rm --no-deps backend python manage.py check --deploy --fail-level WARNING`, without command-line overrides. [deploy/production.env.example](../deploy/production.env.example) is intentionally placeholder-only and must be replaced by a deployment secret manager or equivalent protected configuration.
...
1. **Correction regressions covered.** The focused production truth-policy suite passes, including explicit date-relative surveillance regression coverage; the correction full suite passed **933/933**. Supersession/correction data quality still needs real-source validation.
...
3. Complete model operations: artifact registry, active-model record, promotion approval, rollback target, calibration/threshold review, out-of-time metrics, drift baselines and monitoring jobs.
