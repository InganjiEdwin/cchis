...
| Area | Result |
|---|---|
| Backend Django suite | Baseline: **924 tests discovered; 924 passed**. Correction verification: **937 tests discovered; 937 passed** in 610.734 seconds |
| Surveillance truth/regression tests | Passed; focused production truth-policy coverage is **19/19**, including source-backed records, missing references, cross-ward references, proxy-only labels and superseded references. Explicit `as_of` timestamps keep historical fixtures deterministic while preserving rolling-window cutoffs and truth gates |
...
- Added a central production truth policy that blocks seeded/demo labels, static climate fallback, synthetic feature rows, invalid/unmapped wards, missing or unresolved typed source references, inactive/failed source records, mismatched wards or reporting periods, superseded canonical references and unsafe production model metadata before scores or alerts can be produced. Production model runs must reference a real label dataset; label counts and truth level are recomputed from referenced surveillance records, and proxy evidence cannot satisfy an explicit confirmed-truth claim.
...
| Model feature generation | **Implemented, partially verified** | Features include rainfall, flood indicator, historical cases, seasonality, population proxies, settlement concentration, floodplain exposure, water-body proximity and WASH vulnerability. Feature provenance and truth gates are persisted; production blocks seeded labels, static/synthetic fallback rows, invalid geography, missing/unresolved source references and stale or superseded canonical references. Proxy evidence remains available as caveat context but cannot be asserted as confirmed truth. |
...
1. **Correction regressions covered.** The focused production truth-policy suite passes **19/19**, including source-backed acceptance, missing/cross-ward references, proxy-only confirmed-label rejection, supersession rejection and explicit date-relative surveillance regression coverage; the correction full suite passed **937/937**. Supersession/correction data quality still needs real-source validation.
